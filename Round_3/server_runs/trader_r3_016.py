"""
Enhanced Options Trader - IV Scalping + Mean Reversion
Builds on the 386617 trader with strategies from winning Round 3 team

Key improvements:
1. Volatility smile fitting to detect IV deviations for additional scalping
2. Mean reversion detection on underlying (Velvetfruit Extract)
3. Lightweight mean reversion hedge position
4. Statistical validation of trading opportunities
"""
...
# Configuration matches 386617 with added IV scalping and MR parameters
TRADED_VOUCHERS = {
    "VEV_5000": {
        "strike": 5000,
        "sigma": 0.202,
...
class BlackScholes:
    ...
    def implied_vol_from_price(spot: float, strike: float, tte: float, price: float) -> float:
        """Extract implied volatility from option price via binary search"""
        ...
        for _ in range(50):
            mid = (low + high) / 2.0
            calc_price = BlackScholes.call_price(spot, strike, tte, mid)
            if calc_price < price:
                low = mid
            else:
                high = mid
...
class VolatilitySmile:
    ...
    def fit_smile(self) -> Optional[Tuple[List[float], float]]:
        ...
        # Simple least squares fit for parabola
        sum_m = sum(moneyness)
        ...
        c = (
            (n * (sum_m2 * sum_vm2 - sum_m3 * sum_vm) - sum_m * (sum_m * sum_vm2 - sum_m2 * sum_vm)
            + sum_m2 * (sum_m * sum_vm - sum_m2 * sum_v)) / denom
        )
        b = (
            (n * (sum_m2 * sum_vm2 - sum_m3 * sum_vm) - sum_v * (sum_m * sum_m4 - sum_m2 * sum_m3)
    ...
    def get_fair_iv(self, moneyness: float) -> Optional[float]:
        """Get fair IV from fitted smile at given moneyness"""
    ...
    def get_iv_deviation(self, iv: float, moneyness: float) -> Optional[float]:
        """Get IV deviation: (actual - fair) / fair"""
...
class MeanReversionDetector:
    """Detect mean reversion in underlying asset using EMA and autocorrelation"""
    ...
    def _update_ema(self):
        ...
        if self.ema_value is None:
            self.ema_value = self.price_history[-1]
        else:
            self.ema_value = alpha * self.price_history[-1] + (1 - alpha) * self.ema_value
    ...
    def get_autocorrelation(self, lag: int = 1) -> Optional[float]:
        ...
        returns_1 = rets[:-lag]
        returns_lag = rets[lag:]

        if not returns_1 or not returns_lag:
            return None

        mean_1 = sum(returns_1) / len(returns_1)
        mean_lag = sum(returns_lag) / len(returns_lag)

        cov = sum((r1 - mean_1) * (r2 - mean_lag) for r1, r2 in zip(returns_1, returns_lag))
        var_1 = sum((r - mean_1) ** 2 for r in returns_1)
        var_lag = sum((r - mean_lag) ** 2 for r in returns_lag)

        if var_1 <= 0 or var_lag <= 0:
            return 0.0

        return cov / math.sqrt(var_1 * var_lag)

    def get_mean_reversion_signal(self) -> Tuple[str, float]:
        """
        Returns: (signal_direction, signal_strength)
        signal_direction: 'buy', 'sell', or 'neutral'
...
class Trader:
    """Enhanced trader with IV scalping and mean reversion"""

    # r3_017: skip products that LOST money in 3-day rust backtest of teammate code
    # Per ground truth: HP -54k, K4000 -38k, K5500 -11k, K5400 -4k, K5300 -3k
    BLOCKED_SWING_PRODUCTS = {"HYDROGEL_PACK", "VEV_4000", "VEV_5000", "VEV_5100", "VEV_5200", "VEV_5300", "VEV_5400", "VEV_5500"}
    # === r3_018 OU mean-reversion config (398258 strategy) ===
    ...
    # Hardcoded execution knobs. These still need robustness tuning rather than direct upload fitting.
    CLEAR_BAND = 0.15
    PASSIVE_ONLY_IF_SPREAD_AT_LEAST = 2.0
    MAX_PASSIVE_ABS_POS = 180
    # State-based late aggression: use VE retest plus a tight 5200/5300 surface instead of clock time.
    SECOND_HALF_EDGE_MULT = 0.90
    SECOND_HALF_TAKE_MULT = 1.20

    MR_POSITION_SIZE = 15  # Lightweight mean reversion position
    MR_ENABLED = False
    IV_SCALP_ENABLED = False

    # Hardcoded risk architecture. These are deliberate but still hand-tuned.
    VE_LIMIT = 0
    MAX_ABS_OPTION_DELTA = 999.0
    MAX_STRATEGY_SHORT = 300
    DELTA_HEDGE_THRESHOLD = 999_999.0
    # State-based ramp: arm full-size only after the first VE upside-extension state appears.
    ENABLED_PRODUCTS = {"VEV_5400", "VEV_5500"}
    MAX_SHORT_BY_PRODUCT = {
        "VEV_5100": 300,
        "VEV_5200": 300,
        "VEV_5300": 300,
        "VEV_5400": 300,
    ...
    # Hardcoded campaign-shaping limits. These are still upload-shaped and should be generalized later.
    EARLY_MAX_SHORT_BY_PRODUCT = {
        "VEV_5100": 0,
        "VEV_5200": 0,
        "VEV_5300": 150,
        "VEV_5400": 90,
    ...
    # Hardcoded absolute price gates that still need to be replaced by state/range logic.
    MIN_SELL_PRICE_BY_PRODUCT = {
        "VEV_5100": 183,
        "VEV_5200": 107,
        "VEV_5300": 55,
        "VEV_5400": 18,
    ...
    # State-based second entry: reopen only after VE reaches the retest-up state.
    SECOND_ENTRY_MIN_SELL_PRICE_BY_PRODUCT = {
        "VEV_5100": 182,
        "VEV_5200": 107,
        "VEV_5300": 53,
        "VEV_5400": 16,
    ...
    # Hardcoded absolute profit-take levels. These still need eventual state/range replacement.
    PROFIT_TAKE_ASK_BY_PRODUCT = {
        "VEV_5100": 157,
        "VEV_5200": 88,
        "VEV_5300": 41,
        "VEV_5400": 12,
    ...
    # Swing-book targets. Timestamp windows are removed in 015; activation now comes from the
    # VE-led and HP-specific cycle state machines below.
    SWING_RULES = {
        HYDROGEL: {
            "limit": 200,
            "phases": [
    ...
    # Hardcoded feature windows. These are now used for the full state-based swing book and are
    # calibrated from the explicit window forensics in docs/research/round3_007_*_windows.
    SWING_FEATURE_PRODUCTS = {
        HYDROGEL,
        VE,
        "VEV_4000",
        "VEV_4500",
    ...
    def __init__(self):
        self.volatility_smiles: Dict[str, VolatilitySmile] = {
            product: VolatilitySmile() for product in TRADED_VOUCHERS.keys()
        }
    ...
    def _current_swing_snapshot(self, data: dict, product: str) -> Optional[dict]:
        ...
        if len(prev_prev_history) >= 1:
            ...
            prev_prev_fms = prev_prev_fast - prev_prev_slow
            prev_fms_slope = prev_fms - prev_prev_fms
        mid = history[-1]
        ...
        bottomness = (
            ...
            - 0.20 * rebound
            - 2.50 * fms
            + 12.0 * max(0.0, fms_slope)
            + 10.0 * max(0.0, fms_accel)
        ...
        return {
            ...
            "range_slope": rolling_range - prev_range,
            "fast_minus_slow": fms,
            "fms_slope": fms_slope,
            "fms_accel": fms_accel,
    ...
    def _update_ve_cycle_state(self, data: dict) -> None:
        ...
        extended_up = (
            snap["fast_minus_slow"] >= 0.72
            and snap["drawdown"] <= 3.0
            and snap["rebound"] >= 7.5
            and snap["rolling_range"] >= 8.5
        )
        capitulation = (
            snap["fast_minus_slow"] <= -4.82
            and snap["drawdown"] >= 15.5
            and snap["rebound"] <= 2.5
            and snap["rolling_range"] >= 17.0
        )
        rebound_1 = (
            data.get("ve_has_capitulated", False)
            and snap["fast_minus_slow"] >= 1.5
            and snap["drawdown"] <= 9.0
            and snap["rebound"] >= 6.0
            and snap["rolling_range"] >= 12.0
        )
        retest_up = (
            data.get("ve_has_rebounded", False)
            and snap["fast_minus_slow"] >= 2.61
            and snap["drawdown"] <= 7.0
            and snap["rebound"] >= 14.0
            and snap["rolling_range"] >= 18.0
        )
    ...
    def _update_hp_cycle_state(self, data: dict) -> None:
        ...
        extended_up = (
            snap["fast_minus_slow"] >= 0.0
            and snap["drawdown"] <= 5.0
            and snap["rebound"] >= 6.0
            and snap["rolling_range"] >= 8.0
        )
        capitulation = (
            snap["fast_minus_slow"] <= -10.82
            and snap["drawdown"] >= 38.0
            and snap["rebound"] <= 4.0
            and snap["rolling_range"] >= 39.0
        )
        rebound_short = (
            data.get("hp_has_capitulated", False)
            and snap["fast_minus_slow"] >= 0.0
            and snap["drawdown"] <= 12.0
            and snap["rebound"] >= 21.0
            and snap["rolling_range"] >= 27.0
        )
        bottom_build = (
            snap["fast_minus_slow"] <= -6.0
            and snap["drawdown"] >= 30.0
            and snap["rebound"] <= 12.0
            and snap["rolling_range"] >= 35.0
            and snap["fms_slope"] >= 0.20
            and snap["fms_accel"] >= 0.0
            and snap["drawdown_slope"] <= 0.5
            and snap["bottomness"] >= 35.0
        )
        final_buy = (
            data.get("hp_has_rebounded", False)
            and snap["fast_minus_slow"] <= -8.34
            and snap["drawdown"] >= 22.5
            and snap["rebound"] <= 9.5
            and snap["rolling_range"] >= 29.0
        )
    ...
    def _get_moneyness(self, spot: float, strike: float) -> float:
        """Log moneyness for smile fitting"""
    ...
    def _apply_swing_rules(self, state: TradingState, result: Dict[Symbol, List[Order]], data: dict) -> None:
        for product, rule in self.SWING_RULES.items():
            ...
            pending = sum(order.quantity for order in result.get(product, []))
            pending_buy_at_ask = sum(
                order.quantity for order in result.get(product, [])
                if order.quantity > 0 and order.price == best_ask
            )
            pending_sell_at_bid = sum(
                -order.quantity for order in result.get(product, [])
                if order.quantity < 0 and order.price == best_bid
            )
            position = int(state.position.get(product, 0)) + pending
            ...
            for phase_idx, phase in enumerate(rule["phases"]):
                use_state_gate = product in self.STATEFUL_SWING_CONFIG and phase_idx in self.STATEFUL_SWING_CONFIG[product]
                if use_state_gate:
    ...
    def _ou_pass(self, state, data):
        """398258's OU mean-reversion on K=5000-5300 via deep-ITM vouchers."""
        result = {}
        u_depth = state.order_depths.get("VELVETFRUIT_EXTRACT")
        if u_depth is None or not u_depth.buy_orders or not u_depth.sell_orders:
            return result
        u_bid = max(u_depth.buy_orders); u_ask = min(u_depth.sell_orders)
        ...
        desired = signal
        if dev > self.OU_ENTRY_DEV: desired = -1
        elif dev < -self.OU_ENTRY_DEV: desired = 1

        # Tight surface gate (same as 398258)
        b5200 = state.order_depths.get("VEV_5200")
        b5300 = state.order_depths.get("VEV_5300")
        tight = False
        if b5200 and b5300 and b5200.buy_orders and b5200.sell_orders \
           and b5300.buy_orders and b5300.sell_orders:
            s5200 = min(b5200.sell_orders) - max(b5200.buy_orders)
            s5300 = min(b5300.sell_orders) - max(b5300.buy_orders)
            tight = s5200 <= 2 and s5300 <= 2
        ...
        for product in self.OU_PRODUCTS:
            ...
            if depth is None or not depth.buy_orders or not depth.sell_orders:
                continue
            p_bid = max(depth.buy_orders); p_ask = min(depth.sell_orders)
            bv = abs(int(depth.buy_orders[p_bid])); av = abs(int(depth.sell_orders[p_ask]))
    ...
    def run(self, state: TradingState):
        ...
        # === r3_018 OU pass: 398258 strategy on K=5000-5300 ===
        ...
        # Update mean reversion detector
        ...
        for product, cfg in TRADED_VOUCHERS.items():
            ...
            projected_option_delta += position * BlackScholes.delta(ve_mid, strike, tte, sigma)
        ...
        for product, cfg in TRADED_VOUCHERS.items():
            ...
            strike = int(cfg["strike"])
            sigma = float(cfg["sigma"])
            fair = BlackScholes.call_price(ve_mid, strike, tte, sigma)
            ...
            if not data.get("ve_full_size_armed", False):
                early_max_short = int(self.EARLY_MAX_SHORT_BY_PRODUCT.get(product, max_short))
                max_short = min(max_short, early_max_short)
            sell_room = min(self.VOUCHER_LIMIT + position, max_short + position)
            ...
            if second_entry_armed and product in closed_products:
                min_sell_price = int(
                    self.SECOND_ENTRY_MIN_SELL_PRICE_BY_PRODUCT.get(
            ...
            if self.IV_SCALP_ENABLED:
                moneyness = self._get_moneyness(ve_mid, strike)
            ...
            if spread >= self.PASSIVE_ONLY_IF_SPREAD_AT_LEAST and abs(position) < self.MAX_PASSIVE_ABS_POS:
                ...
                bid_px = int(math.floor(fair - mm_width - 0.75 * inv_skew))
                ask_px = int(math.ceil(fair + mm_width - 0.75 * inv_skew))

                intrinsic_floor = int(math.ceil(intrinsic))
                bid_px = max(intrinsic_floor, min(best_ask - 1, max(best_bid, bid_px)))
                ask_px = max(best_bid + 1, min(best_ask, ask_px))

                if buy_room > 0 and edge_buy > -0.50 * edge and bid_px < best_ask:
                    qty = min(int(cfg["mm_size"]), buy_room)
                    qty = self._cap_delta_qty(qty, option_delta, projected_option_delta)
                    if qty > 0:
                ...
                if sell_room > 0 and edge_sell > -0.50 * edge and ask_px > best_bid and ask_px >= min_sell_price:
                    qty = min(int(cfg["mm_size"]), sell_room)
                    qty = self._cap_delta_qty(qty, -option_delta, projected_option_delta)
                    if qty > 0:
        ...
        target_ve_position = int(round(-projected_option_delta))
        target_ve_position = max(-self.VE_LIMIT, min(self.VE_LIMIT, target_ve_position))
        hedge_qty = target_ve_position - ve_position

        if abs(hedge_qty) >= self.DELTA_HEDGE_THRESHOLD:
            best_bid, best_ask, bid_vol, ask_vol = self._best_bid_ask(ve_depth)
            if hedge_qty > 0 and best_ask is not None:
                qty = min(hedge_qty, ask_vol, self.VE_LIMIT - ve_position)
                if qty > 0:
                    result.setdefault(VE, []).append(Order(VE, best_ask, qty))
            elif hedge_qty < 0 and best_bid is not None:
                qty = min(-hedge_qty, bid_vol, self.VE_LIMIT + ve_position)
                if qty > 0:
                    result.setdefault(VE, []).append(Order(VE, best_bid, -qty))

        # === HIGHER-NOTIONAL SWING BOOK ===
        self._apply_swing_rules(state, result, data)
        # Merge OU orders for K=5000-5300 (override any r3_017 swing orders)
        for product in self.OU_PRODUCTS:
            if product in ou_orders:
                result[product] = ou_orders[product]
            elif product in result:
                # safety net — drop r3_017 orders on OU strikes
                del result[product]
        ...
        if self.MR_ENABLED:
            ...
            current_mr_pos = int(data.get("mr_positions", 0))
            ...
            if mr_signal != "neutral" and mr_strength > 0.3:
                ...
                if ve_depth:
                    ...
                    if best_bid and best_ask:
                        ...
                        if mr_signal == "buy" and current_mr_pos < 100:
                            qty = min(mr_qty, 100 - current_mr_pos)
                            if qty > 0:
                                if VE not in result:
                                    result[VE] = []
                                result[VE].append(Order(VE, best_ask, qty))
                                current_mr_pos += qty
                        
                        elif mr_signal == "sell" and current_mr_pos > -100:
                            qty = min(mr_qty, current_mr_pos + 100)
                            if qty > 0:
                                if VE not in result:
                                    result[VE] = []
                                result[VE].append(Order(VE, best_bid, -qty))
                                current_mr_pos -= qty
                        
                        data["mr_positions"] = current_mr_pos
