...
PEP_CORE_FLOOR      = 75      # keep this long inventory as carry core
PEP_REENTRY_FLOOR   = 72      # if inventory dips below this, re-accumulate
PEP_ENTRY_END_TS    = 25000   # early session fast-entry window
PEP_ENTRY_TAKE_EDGE = 2       # allow crossing ask when edge still positive
PEP_ENTRY_TAKE_CLIP = 8       # max aggressive buy clip per tick
PEP_MM_BUFFER_CLIP  = 5       # max passive buffer sell clip per tick
...
class Trader:
    ...
    def trade_pepper(self, state: TradingState, td: dict) -> tuple:
        ...
        if PEP_NEVER_SELL and not safety_off:
            ...
            if buy_cap > 0 and market_spread > PEP_TAKING_THRESHOLD and best_ask <= fair + PEP_ENTRY_TAKE_EDGE:
