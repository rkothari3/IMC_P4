...
The explanation below assumes familiarity with the basics of Object-Oriented Programming in Python. To refresh this knowledge you can have a look at the source provided on the [Programming resources](https://www.notion.so/Programming-resources-325e8453a0938006a589dbaa2a72a5e3?pvs=21).
...
> ⚠️ **Note**: Observation details help to decide on eventual orders or 'conversion requests', although we expect you won't really need to work much with this class (feel free to skip).
...
2. **Price**: the maximum price at which the algorithm wants to buy (BUY order), or the minimum price at which the algorithm wants to sell (SELL order).
3. **Quantity**: the maximum quantity that the algorithm wishes to buy or sell. If positive, it's a buy order; if negative, it's a sell order.
...
See [Trading glossary](https://www.notion.so/Trading-glossary-325e8453a09380febae2cf77bad14f71?pvs=21) for a more elaborate explanation of order execution in financial markets.
...
⬇️ [Download example.py](attachment:8c87a345-6c7c-40fa-a5ab-a03ff5bd5225:example.py)
...
> **Note**: Observation details help to decide on eventual orders or 'conversion requests', although we expect you won't really need to work much with this class (feel free to skip).
